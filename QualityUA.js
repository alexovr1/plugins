...
(function () {
  ...
  function getBest(results) {
    ...
    for (var i = 0; i < limit; i++) {
      ...
      if (
        ...
        foundRes = "4K";
      else if (title.indexOf("2k") >= 0 || title.indexOf("1440") >= 0)
        foundRes = "2K";
      else if (
      ...
      if (
        foundRes &&
        (!best.resolution ||
          resOrder.indexOf(foundRes) > resOrder.indexOf(best.resolution))
      ) {
        ...
        item.ffprobe.forEach(function (stream) {
          if (stream.codec_type === "video") {
            ...
            if (
              res &&
              (!best.resolution ||
                resOrder.indexOf(res) > resOrder.indexOf(best.resolution))
            ) {
              best.resolution = res;
            }
            if (
              stream.side_data_list &&
              JSON.stringify(stream.side_data_list).indexOf("Vision") >= 0
            )
          ...
          if (stream.codec_type === "audio" && stream.channels) {
            ...
            var aud =
              ch >= 8 ? "7.1" : ch >= 6 ? "5.1" : ch >= 4 ? "4.0" : "2.0";
            if (
              !best.audio ||
              audioOrder.indexOf(aud) > audioOrder.indexOf(best.audio)
            )
              best.audio = aud;
          }
        });
      ...
      if (title.indexOf("vision") >= 0 || title.indexOf("dovi") >= 0)
        best.dolbyVision = true;
      if (title.indexOf("hdr") >= 0) best.hdr = true;
      if (title.indexOf("dub") >= 0 || title.indexOf("дубл") >= 0)
        best.dub = true;
    }
  ...
  function addCardBadges(card, best) {
    ...
    if (best.resolution)
      badges.push(createBadgeImg(best.resolution, true, badges.length));
    if (best.hdr) badges.push(createBadgeImg("HDR", true, badges.length));
    if (best.audio)
      badges.push(createBadgeImg(best.audio, true, badges.length));
    if (best.dub) badges.push(createBadgeImg("DUB", true, badges.length));
    if (best.dolbyVision)
      badges.push(createBadgeImg("Dolby Vision", true, badges.length));
    if (badges.length)
      card
        .find(".card__view")
        .append(
          '<div class="card-quality-badges">' + badges.join("") + "</div>",
        );
  }
      ...
      Lampa.Parser.get(
        ...
        function (response) {
          if (response && response.Results) {
            ...
            var badges = [];
            if (best.ukr)
              badges.push(createBadgeImg("UKR", false, badges.length));
            if (best.resolution)
              badges.push(
                createBadgeImg(best.resolution, false, badges.length),
              );
            if (best.hdr)
              badges.push(createBadgeImg("HDR", false, badges.length));
            if (best.audio)
              badges.push(createBadgeImg(best.audio, false, badges.length));
            if (best.dub)
              badges.push(createBadgeImg("DUB", false, badges.length));
            if (best.dolbyVision)
              badges.push(createBadgeImg("Dolby Vision", false, badges.length));
  ...
  var style =
    ...
    @media (max-width: 768px) {\
      .quality-badges-container { gap: 0.25em; margin: 0 0 1.25em 0; min-height: 1em; }\
