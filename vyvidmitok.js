...
(function () {
  ...
  function initMarksJacRed() {
    ...
    function tryProxies(url, callback) {
      var proxyList =
        workingProxy && workingProxy !== "direct" ? [workingProxy] : proxies;
      ...
      function tryProxy(index) {
        ...
        var target =
          p.indexOf("url=") > -1 ? p + encodeURIComponent(url) : p + url;
    ...
    function getBestJacred(card, callback) {
      ...
      try {
        ...
        if (
          ...
          typeof raw === "object" &&
          raw._ts &&
          Date.now() - raw._ts < 48 * 60 * 60 * 1000
        ) {
      ...
      var apiUrl =
        "https://jr.maxvol.pro/api/v1.0/torrents?search=" +
        ...
        try {
          ...
          results.forEach(function (item) {
            ...
            if (
              ...
              currentRes = "4K";
            else if (t.indexOf("2k") >= 0 || t.indexOf("1440") >= 0)
              currentRes = "2K";
            else if (
              t.indexOf("1080") >= 0 ||
              t.indexOf("fhd") >= 0 ||
            ...
            if (
              resOrder.indexOf(currentRes) > resOrder.indexOf(best.resolution)
            ) {
            ...
            if (
              ...
              best.eng = true;
            if (
              t.indexOf("dolby vision") >= 0 ||
              t.indexOf("dolbyvision") >= 0
            ) {
              best.hdr = true;
    ...
    function checkUafixDirect(movie, callback) {
      ...
      fetchWithProxy(searchUrl, function (err, html) {
        ...
        var hasResults =
          html.indexOf("знайдено") >= 0 && html.indexOf("0 відповідей") < 0;
        callback(hasResults);
      });
    }
    ...
    function renderBadges(container, data, movie) {
      ...
      if (data.resolution && data.resolution !== "SD") {
        if (
          data.resolution === "4K" &&
          Lampa.Storage.get("likhtar_badge_4k", true)
        )
          container.append(createBadge("4k", "4K"));
        else if (
          data.resolution === "FHD" &&
          Lampa.Storage.get("likhtar_badge_fhd", true)
        )
      ...
      if (movie) {
        var rating = parseFloat(
          movie.imdb_rating || movie.kp_rating || movie.vote_average || 0,
        );
        if (rating > 0) {
          var rBadge = document.createElement("div");
          rBadge.classList.add("card__mark", "card__mark--rating");
          rBadge.innerHTML =
            '<span class="mark-star">★</span>' + rating.toFixed(1);
