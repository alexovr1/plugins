  ...
  if (!window.logoplugin) {
    ...
    function applyLogo(img, titleBlock, hasTagline, textHeight) {
      ...
      img.style.paddingTop =
        (Lampa.Storage.get("logo_hide_year", true) ? 0 : 0.3) + "em";
    ...
    Lampa.Listener.follow("full", function (ev) {
      ...
      let details = ev.object.activity
        .render()
        .find(".full-start-new__details");
      let taglineBlock = ev.object.activity
        .render()
        .find(".full-start-new__tagline");

      let hasTagline =
        taglineBlock.length > 0 && taglineBlock.text().trim() !== "";
      let titleElem = block[0];

      let lang =
        Lampa.Storage.get("logo_lang", "") || Lampa.Storage.get("language");
      let size = Lampa.Storage.get("logo_size", "original");

      let cacheKey =
        "logo_cache_width_based_v1_" + type + "_" + card.id + "_" + lang;
      ...
      function moveHeadWithoutAnimation() {
        if (Lampa.Storage.get("logo_hide_year", true)) {
          if (
            head.length &&
            details.length &&
            details.find(".logo-moved-head").length === 0
          ) {
            let h = head.html();
            if (h) {
      ...
      $.get(url, function (data) {
        ...
        let src = Lampa.TMDB.image(
          "/t/p/" + size + file.replace(".svg", ".png"),
        );
